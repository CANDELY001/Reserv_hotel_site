    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
         ...
         if(request.getSession().getAttribute("v")!=null){
             try {
                 ...
                 PreparedStatement st=con.prepareStatement("insert into saved values(?,?)");
                 st.setInt(1,Integer.parseInt(request.getParameter("idch")));
                 st.setString(2,v.getPseudo());
                 st.executeUpdate();
                 System.out.println("liiike");
                request.getRequestDispatcher("//jsp/ChambreandTarif.jsp");
             } catch (Exception ex) {
